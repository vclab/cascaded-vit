...
class CFFN(torch.nn.Module):
    def __init__(self, ed: int, h: int, resolution: int, num_chunks: int = 2):
        super().__init__()
        assert ed % num_chunks == 0

        self.num_chunks = num_chunks
        self.chunk_dim = ed // num_chunks
        self.hidden_chunk = h // num_chunks

        # build one small FFN per chunk
        self.chunk_ffn = torch.nn.ModuleList([
            FFN(self.chunk_dim, self.hidden_chunk, resolution)
            for _ in range(num_chunks)
        ])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # split into num_chunks along channel axis
        chunks = x.chunk(self.num_chunks, dim=1)

        # apply each small FFN independently
        out_chunks = []
        prev = None

        # cascade the ffns
        for idx, chunk in enumerate(chunks):
            inp = chunk if prev is None else chunk + prev
            out = self.chunk_ffn[idx](inp)
...
class CascadedViTBlock(torch.nn.Module):
    ...
    def __init__(self, type,
        ...
        self.ffn0 = Residual(CFFN(ed, int(ed * 2.5), resolution))
        ...
        self.ffn1 = Residual(CFFN(ed, int(ed * 2.5), resolution))
        ...
        for i, (stg, ed, kd, dpth, nh, ar, wd, do) in enumerate(
            ...
            if do[0] == 'subsample':
                ...
                blk.append(torch.nn.Sequential(Residual(Conv2d_BN(embed_dim[i], embed_dim[i], 3, 1, 1, groups=embed_dim[i], resolution=resolution)),
                                    Residual(CFFN(embed_dim[i], int(embed_dim[i] * 2.5), resolution)),))
                ...
                blk.append(torch.nn.Sequential(Residual(Conv2d_BN(embed_dim[i + 1], embed_dim[i + 1], 3, 1, 1, groups=embed_dim[i + 1], resolution=resolution)),
                                    Residual(CFFN(embed_dim[i + 1], int(embed_dim[i + 1] * 2.5), resolution)),))
